...
class Cluster(BaseModel):
    ...
    description: Optional[str] = Field(
        default=None, description="Detailed description of the cluster topic"
    )
    created_at: Optional[datetime] = None
    post_count: int = Field(default=0, description="Number of posts in this cluster")
    centroid_blob: Optional[bytes] = Field(
        default=None, description="Serialized cluster centroid as bytes"
    )
    ...
    def __repr__(self) -> str:
        ...
        return f"Cluster(id={self.id}, label='{self.label}', " f"post_count={self.post_count})"
